...
def test_dev_cycle_pre_promotion(setup_git_repo, monkeypatch, tmp_path_factory) -> None:
    ...
    expected = rf"""
    * {config.stable}: (projectA) add hotfix -  (HEAD -> {config.stable}, origin/{config.stable})
    | * {config.beta}: (projectA) add feature 1 -  (tag: projectA-1.1.0b0, origin/{config.beta}, {config.beta})
    |/  
    * initial state -  (tag: projectB-1.0.0, tag: projectA-1.0.0, origin/{config.rc}, {config.rc})"""
    ...
    expected = rf"""
    * {config.stable}: (projectA) add hotfix -  (HEAD -> {config.stable}, tag: projectA-1.0.1, origin/{config.rc}, origin/{config.stable})
    | * {config.beta}: (projectA) add feature 1 -  (tag: projectA-1.1.0b0, origin/{config.beta}, {config.beta})
    |/  
    * initial state -  (tag: projectB-1.0.0, tag: projectA-1.0.0, {config.rc})"""
    verify_git_graph(expected)
    ...
    # same as before. no new tag
    assert latest_tag("projectA-*") == "projectA-1.0.1"

    expected = rf"""
    *   auto-hotfix into {config.beta}: {config.stable}: (projectA) add hotfix -  (origin/{config.beta})
    |\  
    | * {config.stable}: (projectA) add hotfix -  (HEAD -> {config.rc}, tag: projectA-1.0.1, origin/{config.rc}, origin/{config.stable}, {config.stable})
    * | {config.beta}: (projectA) add feature 1 -  (tag: projectA-1.1.0b0, {config.beta})
    |/  
    * initial state -  (tag: projectB-1.0.0, tag: projectA-1.0.0)"""
    verify_git_graph(expected)
    ...
    expected = rf"""
    *   auto-hotfix into {config.beta}: {config.stable}: (projectA) add hotfix -  (HEAD -> {config.beta}, tag: projectA-1.1.0b1, origin/{config.beta})
    |\  
    | * {config.stable}: (projectA) add hotfix -  (tag: projectA-1.0.1, origin/{config.rc}, origin/{config.stable}, {config.rc}, {config.stable})
